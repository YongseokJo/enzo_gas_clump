...
/// by Jo this parameters are for gas clump initialization
...
float GCSphere (float x, float y ,float z, float xc, float yc ,float zc);
...
private:
  ...
  int RefineAtStart;
	FLOAT GCCenter[MAX_DIMENSION];
	FLOAT GCRadius;
	FLOAT GCMass;
	FLOAT	GCDensity;
	FLOAT GCVelocity[MAX_DIMENSION];
	int GasClumpOn;
...
public:
    ...
    HierarchyEntry &TopGrid, TopGridData &MetaData)
  {
    ...
    char *BzName = "Bz";
    char *PhiName = "Phi";
    char *Extra0Name = "PassiveScalar1"; // by Jo YS
    char *Extra1Name = "PassiveScalar2"; // by Jo YS
    ...
    while (fgets(line, MAX_LINE_LENGTH, fptr) != NULL)
    {
      ...
      ret += sscanf(line, "AgoraRestartPassiveScalarOn  = %"ISYM,
		    &TestProblemData.PassiveScalarOn);
      ret += sscanf(line, "AgoraRestartGasClumpCenter  = %"FSYM" %"FSYM" %"FSYM,
		    GCCenter, GCCenter+1, GCCenter+2);
      ret += sscanf(line, "AgoraRestartGasClumpVelocity  = %"FSYM" %"FSYM" %"FSYM,
		    GCVelocity, GCVelocity+1, GCVelocity+2);
      ret += sscanf(line, "AgoraRestartGasClumpRadius  = %"FSYM,
		    &GCRadius);
      ret += sscanf(line, "AgoraRestartGasClumpMass  = %"FSYM,
		    &GCMass);
      ret += sscanf(line, "AgoraRestartGasClumpOn  = %"ISYM,
		    &GasClumpOn);
    ...
    if (TestProblemData.UseMetallicityField)
		...
		if (TestProblemData.PassiveScalarOn) {// if (PassiveScalarOn) by Jo
			DataLabel[count++] = Extra0Name;
			DataLabel[count++] = Extra1Name;
		}
    ...
    {
			PassiveScalarOn = TestProblemData.PassiveScalarOn;
      ...
      fprintf(Outfptr, "AgoraRestartUseMetallicityField  = %"ISYM"\n",
	      TestProblemData.UseMetallicityField);
      fprintf(Outfptr, "AgoraRestartPassiveScalarOn  = %"ISYM"\n",
	      TestProblemData.PassiveScalarOn);
		...
		TopGridData &MetaData)
{
	...
	// by Jo YS
	int Extra0Num, Extra1Num;
	if (TestProblemData.PassiveScalarOn) {
		Extra0Num = FindField(ExtraType0, thisgrid->FieldType, thisgrid->NumberOfBaryonFields);
		Extra1Num = FindField(ExtraType1, thisgrid->FieldType, thisgrid->NumberOfBaryonFields);
		//fprintf(stderr,"%d\n", ExtraNum);
	}
	...
	HaloDensity = this->HaloMass / BoxVolume;  // The average density of the given halo mass and size
	...
	for (k = 0; k < thisgrid->GridDimension[2]; k++)
	{
		...
		{
			for (i = 0; i < thisgrid->GridDimension[0]; i++, index++) // goes over cells
			{
				/* Compute position */
				...
				radius = sqrt(POW(x, 2) +  // from the center of the simulation
						POW(y, 2) +
						POW(z, 2) );

				xy_radius = sqrt(POW(x, 2) +  // in the z plane
						POW(y, 2) );

				/* Find disk density, halo density and internal energy */

				DiskDensity = gauss_mass(RhoZero, x/LengthUnits, y/LengthUnits,
						z/LengthUnits, cellwidth) / POW(cellwidth, 3); // Disk density with respect to position
				...
				if ( HaloDensity*HaloTemperature > DiskDensity*DiskTemperature )
				{
					...
					// by Jo YS
					if (TestProblemData.PassiveScalarOn && this->GasClumpOn) {
						thisgrid->BaryonField[Extra0Num][index] = 1e-10*HaloDensity; // Background
						thisgrid->BaryonField[Extra1Num][index] = 1e-10*HaloDensity; // Background
					}
				...
				else // Ok, we're in the disk
				{
					...
					if (TestProblemData.PassiveScalarOn && this->GasClumpOn) {
						thisgrid->BaryonField[Extra0Num][index] = 1e-10*HaloDensity; // Background
						thisgrid->BaryonField[Extra1Num][index] = DiskDensity; // Passive Scalar For Disk
					}
				...
				// Position criteria by Jo
				if (this->GasClumpOn && GCSphere(x/LengthUnits,y/LengthUnits,z/LengthUnits,
						 	this->GCCenter[0], this->GCCenter[1], this->GCCenter[2]) < this->GCRadius*this->GCRadius) {
					thisgrid->BaryonField[DensNum][index] = this->GCDensity; // 1e8 clump of 1 kpc radius
					if (TestProblemData.PassiveScalarOn) {
						thisgrid->BaryonField[Extra0Num][index] = this->GCDensity; // 1e8 clump of 1 kpc radius
						thisgrid->BaryonField[Extra1Num][index] = 1e-10*HaloDensity; // 1e8 clump of 1 kpc radius
					}
					thisgrid->BaryonField[Vel1Num][index] = this->GCVelocity[0]; // in code units
					thisgrid->BaryonField[Vel2Num][index] = this->GCVelocity[1]; // in code units
					thisgrid->BaryonField[Vel3Num][index] = this->GCVelocity[2]; // in code units
				}
