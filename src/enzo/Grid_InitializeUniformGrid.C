				...
				float UniformCR)
{
  ...
  int ExtraField[2];
	...
	fprintf(stderr,"Here?%d", TestProblemData.PassiveScalarOn);
  /* create fields */
  ...
  // as two extra fields
	if (TestProblemData.UseMetallicityField) {
		...
		if(TestProblemData.MultiMetals){ // by Jo YS added PassiveScalarOn
			FieldType[ExtraField[0] = NumberOfBaryonFields++] = ExtraType0;
			FieldType[ExtraField[1] = NumberOfBaryonFields++] = ExtraType1;
		}
	}
	fprintf(stderr,"Here?%d", TestProblemData.PassiveScalarOn);
	if(TestProblemData.PassiveScalarOn){ // by Jo YS added PassiveScalarOn
		fprintf(stderr, "Passive On for FieldType");
		FieldType[ExtraField[0] = NumberOfBaryonFields++] = ExtraType0;
		FieldType[ExtraField[1] = NumberOfBaryonFields++] = ExtraType1;
	}
  ...
  for (i = 0; i < size; i++){
	...
	BaryonField[MetalIaNum][i] = TestProblemData.MetallicitySNIaField_Fraction*
	  UniformDensity;
			...
			if (TestProblemData.PassiveScalarOn) { // by Jo YS
				BaryonField[ExtraField[0]][i] = 1e-10 * UniformDensity;
				BaryonField[ExtraField[1]][i] = 1e-10 * UniformDensity;
			}
		} // if(TestProblemData.UseMetallicityField)
